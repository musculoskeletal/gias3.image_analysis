...
class CLMSegmentation( object ):
	...
	def _sampleAndRegressFilter(self, Li, L):
		...
		samplePointsTemp = [disp[i]+L[i] for i in xrange(len(L))]
		regressResults = []
		validLi = []
		samplePoints = []
	...
	def _processVotesFilter(self, regressResults, samplePoints):
		...
		maskPoints = []
		for i in xrange(len(regressResults)):
	...
	def _filterValidLandmarks(self, landmarks):
		...
		if self.hasMaskedImage:
			validLandmarkIndices = [li for li in landmarkIndices if ( self.HRVImage.checkIndexInBounds(inds[li]) and not self.HRVImage.checkIndexIsMasked(inds[li]) ) ]
	...
	def segment( self, meshParams0, verbose=1, debug=0 ):
		...
		while it<self.params.maxIt:
			...
			if self.filterLandmarks:
				regressResults, samplePoints, goodLandmarkIndices = self._sampleAndRegressFilter(validLandmarkIndices, validLandmarks)
